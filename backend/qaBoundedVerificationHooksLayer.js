...
function buildBoundedQaHookCatalog(bridge = {}, qaRuntime = {}) {
  const hooks = qaRuntime.qa_execution_hooks || {};
  return {
    hook_catalog_kind: 'bounded_qa_verification_hook_catalog',
    lane: 'qa',
    hooks: [
      {
      ...
      {
        hook: hooks.validate_execution_contract?.hook || 'assert_qa_contract_boundaries',
        mode: hooks.validate_execution_contract?.mode || 'guardrail',
        executable: false,
      },
      {
        hook: hooks.stage_qa_execution?.hook || 'plan_qa_verification_noop',
        mode: hooks.stage_qa_execution?.mode || 'hold',
        executable: false,
      },
      {
      ...
      {
        hook: hooks.emit_execution_evidence?.hook || 'emit_qa_execution_report',
...
function buildBoundedQaHookGuardrails(bridge = {}, session = {}) {
  return {
    read_only_default: true,
    bounded_qa_hooks_only: true,
    verification_first_semantics: true,
...
function buildBoundedQaHookSummary(catalog = {}) {
  return {
    lane: 'qa',
    ...
    verification_first: catalog.verification_first === true,
    explainable: true,
  };
}

function buildBoundedQaHookPayload(input = {}) {
  return {
...
function buildQaBoundedVerificationHooksLayer(runtimeState = {}, actorRole = 'orchestrator') {
  ...
  const boundedQaHookSummary = buildBoundedQaHookSummary(boundedQaHookCatalog);
  const boundedQaHookPayload = buildBoundedQaHookPayload({ actorRole, session, decisionAssistance, knowledge, catalog: boundedQaHookCatalog });

  return {
    updatedAt,
    actor_role: actorRole,
    hook_surface_kind: 'qa-bounded-verification-hooks',
    qa_bounded_verification_hooks: {
      hook_surface_kind: 'controlled_qa_bounded_verification_hooks',
      runtime_source: 'supabase',
      governed: true,
      read_only_default: true,
      bridge_summary: bridge.execution_bridge_summary,
      qa_runtime_summary: qaRuntime.qa_runtime_summary,
      decision_assistance: decisionAssistance.planning_output,
      knowledge_routing: knowledge.routing_summary,
    },
    bounded_qa_hook_catalog: boundedQaHookCatalog,
    bounded_qa_hook_guardrails: boundedQaHookGuardrails,
    bounded_qa_hook_summary: boundedQaHookSummary,
    bounded_qa_hook_payload: boundedQaHookPayload,
  };
}

module.exports = {
  buildQaBoundedVerificationHooksLayer,
  buildBoundedQaHookCatalog,
  buildBoundedQaHookGuardrails,
  buildBoundedQaHookSummary,
  buildBoundedQaHookPayload,
};
