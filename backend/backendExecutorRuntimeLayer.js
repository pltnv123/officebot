...
function buildBackendExecutionPlan(input = {}) {
  const contract = input.runtimeContracts?.backend_runtime_contract || {};
  const envelope = input.envelope?.backend_execution_envelope || {};
  const taskFrames = Array.isArray(input.handoffContracts?.backend_handoff_contract?.task_framing)
    ? input.handoffContracts.backend_handoff_contract.task_framing
    : [];

  return {
    plan_kind: 'backend_controlled_execution_plan',
    ...
    lane: 'backend',
    allowed_actions: contract.allowed_actions || ['analyze_backend_scope', 'prepare_backend_patch', 'report_backend_artifacts'],
    ...
    staged_steps: [
      ...
      },
      {
        ...
        status: contract.authorized ? 'ready' : 'hold',
        hook: 'assert_backend_contract_boundaries',
      },
      {
        stage: 'dry_scope_analysis',
        status: contract.authorized ? 'ready' : 'hold',
        hook: 'plan_backend_changes_noop',
      },
      {
        stage: 'operator_checkpoint',
        status: 'ready',
      ...
      },
      {
        ...
        status: 'ready',
        hook: 'emit_backend_execution_report',
      },
    ],
...
function buildBackendExecutionConstraints(input = {}) {
  const contract = input.runtimeContracts?.backend_runtime_contract || {};
...
function buildBackendExecutionHooks(input = {}) {
  const authorized = input.runtimeContracts?.backend_runtime_contract?.authorized === true;
...
function buildBackendRuntimeSummary(input = {}) {
  ...
  return {
    ...
    simulation_posture: input.simulation?.backend_launch_simulation?.execution_framing?.simulated_launch_posture || 'simulated_hold',
    reconciliation_outcome: input.reconciliation?.reconciliation_outcome?.outcome || 'lane_reconciliation_hold',
    explainable: true,
  };
}

function buildBackendRuntimePayload(input = {}) {
  const contract = input.runtimeContracts?.backend_runtime_contract || {};
  ...
  return {
    ...
    simulated_launch_posture: input.simulation?.backend_launch_simulation?.execution_framing?.simulated_launch_posture || 'simulated_hold',
    governance_outcome: contract.runtime_inputs?.governance_outcome || 'dispatch_hold_decision',
...
function buildBackendRuntimeInputPack(runtimeState = {}, actorRole = 'orchestrator') {
  ...
  const backendContract = runtimeContracts.backend_runtime_contract || {};
  const authorized = backendContract.authorized === true;
  const governanceOutcome = backendContract.runtime_inputs?.governance_outcome || session.session_audit_context?.governance_outcome || authorization.authorization_payload?.governance_outcome || releaseDecision.release_decision_payload?.governance_outcome || 'dispatch_hold_decision';
  ...
  const scope = backendContract.executable_payload?.scope || [];
  const blockers = backendContract.executable_payload?.blockers || (authorized ? [] : ['not_authorized_for_backend_execution']);
  const dependencies = backendContract.executable_payload?.dependencies || [];
  ...
  return {
    ...
    handoffContracts: {
      backend_handoff_contract: {
        task_framing: (session.session_lanes || []).filter((entry) => entry.lane === 'backend').map((entry) => ({
    ...
    reconciliation: {
      reconciliation_outcome: {
        outcome: backendContract.runtime_inputs?.reconciliation_outcome || session.execution_session_payload?.reconciliation_outcome || authorization.authorization_payload?.reconciliation_outcome || releaseDecision.release_decision_payload?.reconciliation_outcome || 'lane_reconciliation_hold',
      },
    },
    laneHandoff: {
      lane_handoff_summary: {
        ready_lanes: backendContract.runtime_inputs?.lane_handoff_ready || session.execution_session_payload?.lane_handoff_ready || authorization.authorization_payload?.lane_handoff_ready || releaseDecision.release_decision_payload?.lane_handoff_ready || 0,
      },
...
function buildBackendExecutorRuntimeLayer(runtimeState = {}, actorRole = 'orchestrator') {
  ...
  const inputPack = buildBackendRuntimeInputPack(baseRuntime, actorRole);
  ...
  const backendExecutionPlan = buildBackendExecutionPlan({
    session,
    runtimeContracts,
    envelope,
    handoffContracts,
  });
  const backendExecutionConstraints = buildBackendExecutionConstraints({
    runtimeContracts,
    authorization,
    releaseDecision,
  });
  const backendExecutionHooks = buildBackendExecutionHooks({
    session,
    runtimeContracts,
  });
  const backendRuntimeSummary = buildBackendRuntimeSummary({
  ...
  const backendRuntimePayload = buildBackendRuntimePayload({
    actorRole,
    session,
    authorization,
    releaseDecision,
  ...
  return {
    ...
    runtime_layer_kind: 'backend-executor-runtime',
    backend_executor_runtime: {
      lane: 'backend',
      runtime_kind: 'controlled_backend_executor_runtime',
      session_id: session.execution_session?.session_id || null,
      runtime_source: 'supabase',
      governed: true,
      read_only_default: true,
      execution_mode: backendExecutionPlan.execution_mode,
      authorization_outcome: authorization.execution_authorization_outcome?.outcome || 'authorization_withheld',
      release_decision: releaseDecision.release_decision_outcome?.decision || 'release_hold_decision',
      contract: runtimeContracts.backend_runtime_contract || null,
      knowledge_routing: knowledge.routing_summary,
      decision_assistance: decisionAssistance.planning_output,
    },
    backend_execution_plan: backendExecutionPlan,
    backend_execution_constraints: backendExecutionConstraints,
    backend_execution_hooks: backendExecutionHooks,
    backend_runtime_summary: backendRuntimeSummary,
    backend_runtime_payload: backendRuntimePayload,
  };
}

module.exports = {
  buildBackendExecutorRuntimeLayer,
  buildBackendExecutionPlan,
  buildBackendExecutionConstraints,
  buildBackendExecutionHooks,
  buildBackendRuntimeSummary,
  buildBackendRuntimePayload,
};
