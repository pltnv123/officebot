"""
SQLite database for OfficeBot — Unified AI Assistant
"""

import sqlite3
import os
from datetime import datetime
from typing import Optional

from config import DB_PATH, FREE_REQUESTS


def get_connection() -> sqlite3.Connection:
    """Get a database connection, creating tables if needed."""
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    _create_tables(conn)
    return conn


def _create_tables(conn: sqlite3.Connection):
    """Create all tables if they don't exist."""
    c = conn.cursor()

    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            user_id        INTEGER PRIMARY KEY,
            username       TEXT DEFAULT '',
            first_name     TEXT DEFAULT '',
            free_requests  INTEGER DEFAULT 10,
            total_requests INTEGER DEFAULT 0,
            is_premium     INTEGER DEFAULT 0,
            created_at     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            last_active    TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    """)

    c.execute("""
        CREATE TABLE IF NOT EXISTS reminders (
            id             INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id        INTEGER NOT NULL,
            reminder_text  TEXT    NOT NULL,
            remind_time    TEXT    NOT NULL,
            is_sent        INTEGER DEFAULT 0,
            created_at     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(user_id)
        )
    """)

    c.execute("""
        CREATE TABLE IF NOT EXISTS notes (
            id          INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id     INTEGER NOT NULL,
            title       TEXT    NOT NULL,
            raw_notes   TEXT    DEFAULT '',
            summary     TEXT    DEFAULT '',
            created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(user_id)
        )
    """)

    c.execute("""
        CREATE TABLE IF NOT EXISTS schedule (
            id          INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id     INTEGER NOT NULL,
            event       TEXT    NOT NULL,
            event_time  TEXT    NOT NULL,
            notes       TEXT    DEFAULT '',
            created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(user_id)
        )
    """)

    conn.commit()


# ═══════════════════════════════════════════════════════════════════════════
# USER OPERATIONS
# ═══════════════════════════════════════════════════════════════════════════

def get_or_create_user(user_id: int, username: str = "", first_name: str = "") -> dict:
    """Get user or create if new."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("SELECT * FROM users WHERE user_id = ?", (user_id,))
    row = c.fetchone()

    if row:
        c.execute(
            "UPDATE users SET last_active = CURRENT_TIMESTAMP WHERE user_id = ?",
            (user_id,)
        )
        conn.commit()
        user = dict(row)
    else:
        c.execute(
            "INSERT INTO users (user_id, username, first_name, free_requests) VALUES (?, ?, ?, ?)",
            (user_id, username, first_name, FREE_REQUESTS)
        )
        conn.commit()
        user = {
            "user_id": user_id,
            "username": username,
            "first_name": first_name,
            "free_requests": FREE_REQUESTS,
            "total_requests": 0,
            "is_premium": 0,
        }

    conn.close()
    return user


def use_request(user_id: int) -> bool:
    """Try to consume one request. Returns True if successful."""
    conn = get_connection()
    c = conn.cursor()

    c.execute("SELECT free_requests FROM users WHERE user_id = ?", (user_id,))
    row = c.fetchone()
    if row and row[0] > 0:
        c.execute(
            "UPDATE users SET free_requests = free_requests - 1, total_requests = total_requests + 1 WHERE user_id = ?",
            (user_id,)
        )
        conn.commit()
        conn.close()
        return True

    conn.close()
    return False


def add_requests(user_id: int, count: int):
    """Add requests to user balance."""
    conn = get_connection()
    conn.execute(
        "UPDATE users SET free_requests = free_requests + ? WHERE user_id = ?",
        (count, user_id)
    )
    conn.commit()
    conn.close()


def get_user_quota(user_id: int) -> dict:
    """Get user's request quota."""
    conn = get_connection()
    c = conn.cursor()
    c.execute("SELECT free_requests, total_requests FROM users WHERE user_id = ?", (user_id,))
    row = c.fetchone()
    conn.close()

    if row:
        return {"free_requests": row[0], "total_requests": row[1]}
    return {"free_requests": FREE_REQUESTS, "total_requests": 0}


# ═══════════════════════════════════════════════════════════════════════════
# REMINDER OPERATIONS
# ═══════════════════════════════════════════════════════════════════════════

def add_reminder(user_id: int, text: str, remind_time: str) -> int:
    """Add a reminder. Returns the reminder ID."""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        "INSERT INTO reminders (user_id, reminder_text, remind_time) VALUES (?, ?, ?)",
        (user_id, text, remind_time)
    )
    conn.commit()
    rid = c.lastrowid
    conn.close()
    return rid


def get_pending_reminders() -> list[dict]:
    """Get all due, unsent reminders."""
    conn = get_connection()
    c = conn.cursor()
    now = datetime.now().isoformat()
    c.execute(
        """SELECT r.*, u.username, u.first_name
           FROM reminders r
           JOIN users u ON r.user_id = u.user_id
           WHERE r.is_sent = 0 AND r.remind_time <= ?
           ORDER BY r.remind_time""",
        (now,)
    )
    rows = [dict(r) for r in c.fetchall()]
    conn.close()
    return rows


def mark_reminder_sent(reminder_id: int):
    """Mark a reminder as sent."""
    conn = get_connection()
    conn.execute("UPDATE reminders SET is_sent = 1 WHERE id = ?", (reminder_id,))
    conn.commit()
    conn.close()


def get_user_reminders(user_id: int, upcoming_only: bool = True) -> list[dict]:
    """Get reminders for a user."""
    conn = get_connection()
    c = conn.cursor()
    now = datetime.now().isoformat()

    if upcoming_only:
        c.execute(
            "SELECT * FROM reminders WHERE user_id = ? AND is_sent = 0 AND remind_time > ? ORDER BY remind_time",
            (user_id, now)
        )
    else:
        c.execute(
            "SELECT * FROM reminders WHERE user_id = ? ORDER BY remind_time DESC LIMIT 20",
            (user_id,)
        )

    rows = [dict(r) for r in c.fetchall()]
    conn.close()
    return rows


# ═══════════════════════════════════════════════════════════════════════════
# NOTES OPERATIONS
# ═══════════════════════════════════════════════════════════════════════════

def add_note(user_id: int, title: str, raw_notes: str = "", summary: str = "") -> int:
    """Add a note. Returns note ID."""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        "INSERT INTO notes (user_id, title, raw_notes, summary) VALUES (?, ?, ?, ?)",
        (user_id, title, raw_notes, summary)
    )
    conn.commit()
    nid = c.lastrowid
    conn.close()
    return nid


def update_note_summary(note_id: int, raw_notes: str, summary: str):
    """Update note with raw notes and AI summary."""
    conn = get_connection()
    conn.execute(
        "UPDATE notes SET raw_notes = ?, summary = ? WHERE id = ?",
        (raw_notes, summary, note_id)
    )
    conn.commit()
    conn.close()


def get_user_notes(user_id: int, limit: int = 10) -> list[dict]:
    """Get recent notes for a user."""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        "SELECT * FROM notes WHERE user_id = ? ORDER BY created_at DESC LIMIT ?",
        (user_id, limit)
    )
    rows = [dict(r) for r in c.fetchall()]
    conn.close()
    return rows


# ═══════════════════════════════════════════════════════════════════════════
# SCHEDULE OPERATIONS
# ═══════════════════════════════════════════════════════════════════════════

def add_schedule_event(user_id: int, event: str, event_time: str, notes: str = "") -> int:
    """Add a schedule event. Returns event ID."""
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        "INSERT INTO schedule (user_id, event, event_time, notes) VALUES (?, ?, ?, ?)",
        (user_id, event, event_time, notes)
    )
    conn.commit()
    eid = c.lastrowid
    conn.close()
    return eid


def get_user_schedule(user_id: int, date: str = "") -> list[dict]:
    """Get upcoming schedule events for a user."""
    conn = get_connection()
    c = conn.cursor()
    now = datetime.now().isoformat()

    if date:
        c.execute(
            "SELECT * FROM schedule WHERE user_id = ? AND date(event_time) = date(?) ORDER BY event_time",
            (user_id, date)
        )
    else:
        c.execute(
            "SELECT * FROM schedule WHERE user_id = ? AND event_time >= ? ORDER BY event_time LIMIT 20",
            (user_id, now)
        )

    rows = [dict(r) for r in c.fetchall()]
    conn.close()
    return rows
