...
STATE_OUT="$SCRIPT_DIR/state.json"
RUNTIME_DB="$SCRIPT_DIR/.runtime_progress.json"
...
def walk(node, task_id, path):
    ...
    if children:
        ...
        if status == "doing" and est > 0:
            # smooth factual growth based on elapsed wall time
...
cpu = "0.00"
try:
    cmd = "ps -C openclaw-gateway -o %cpu= 2>/dev/null"
    out = subprocess.check_output(cmd, shell=True, text=True).strip().splitlines()
    s = sum(float(x.strip() or 0) for x in out)
    cpu = f"{s:.2f}"
except Exception:
    pass
