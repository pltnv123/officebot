#!/usr/bin/env node
/**
 * Smoke test: Delivery page grouped file tree + multi-file editing
 * 
 * Tests:
 * 1. Generate script artifact
 * 2. Open delivery page (HTTP 200)
 * 3. File tree has grouped structure
 * 4. Groups: Source Code, Documentation, Data & Samples, Execution Output
 * 5. Editable badge for script.py
 * 6. Read-only badge for other files
 * 7. Run Script works
 * 8. Run Edited works
 * 9. Download ZIP works
 * 10. Run History loads
 */
...
async function runSmoke() {
  console.log('🧪 WEBSTUDIO-DELIVERY-GROUPED-FILETREE SMOKE\n');
  ...
  console.log('   ✅ Delivery page opens');
  
  // Step 4: Check grouped file tree structure
  console.log('\n4. Testing grouped file tree...');
  const requiredGroups = ['Source Code', 'Documentation', 'Data \u0026 Samples', 'Execution Output'];
  let missingGroups = [];
  requiredGroups.forEach(group => {
    if (!deliveryHtml.includes(group)) {
      missingGroups.push(group);
    }
  });
  if (missingGroups.length > 0) {
    console.error('❌ Missing file groups:', missingGroups.join(', '));
    process.exit(1);
  }
  console.log('   ✅ File groups present:', requiredGroups.join(', '));
  
  // Step 5: Check file-group CSS classes
  console.log('\n5. Testing file-group CSS...');
  if (!deliveryHtml.includes('file-group') || !deliveryHtml.includes('file-group-header')) {
    console.error('❌ Missing file-group CSS classes');
    process.exit(1);
  }
  console.log('   ✅ File-group CSS present');
  
  // Step 6: Check editable badge for script.py
  console.log('\n6. Testing editable badges...');
  if (!deliveryHtml.includes('file-badge editable')) {
    console.error('❌ Missing editable badge');
    process.exit(1);
  }
  if (!deliveryHtml.includes('file-badge">read-only')) {
    console.error('❌ Missing read-only badge');
    process.exit(1);
  }
  console.log('   ✅ Editable and read-only badges present');
  
  // Step 7: Run Script (original)
  console.log('\n7. Testing Run Script (original)...');
